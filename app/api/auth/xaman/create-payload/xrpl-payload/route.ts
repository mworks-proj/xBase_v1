...
export async function POST(request: NextRequest): Promise<NextResponse<PayloadResponse>> {
  try {
    ...
    if (!supabaseUrl || !anonKey) {
      ...
      return NextResponse.json({ ok: false, error: "Server configuration error" }, { status: 400 })
    }
    ...
    // Call XRPL-specific Edge Function
    const edgeFunctionUrl = `${supabaseUrl}/functions/v1/xaman-createPayload-xrpl`
